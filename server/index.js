...
app.get("/api/admin/galeria/albums", authenticateToken, async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM galeria_albums ORDER BY created_at DESC");
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put("/api/admin/galeria/albums/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE galeria_albums SET ${fields.join(", ")} WHERE id = $${idx} RETURNING *`,
      values
    );
...
app.delete("/api/admin/galeria/albums/:id", authenticateToken, async (req, res) => {
  try {
    await pool.query("DELETE FROM galeria_albums WHERE id = $1", [req.params.id]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.listen(PORT, async () => {
  ...
  // Migración: tabla galería de álbumes (links a Google Photos)
  try {
    await pool.query(`
